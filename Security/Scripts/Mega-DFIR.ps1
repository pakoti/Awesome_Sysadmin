...
$Version = '1.0.0'
Write-Host "Mega Script for DFIR"
Write-Host "Version: $Version"
Write-Host "Developed By github.com/pakoti"
Write-Host "===========================================`n"
...
#function for collecting open ports
...
#function for autorun informations and appas that are in autorun (persistance)
...
#looking for installed drives(good for findinf info about unkown installed drivers)
...
function Get-ChromeFiles {
    ...
    if (Test-Path "C:\Users\$Username\AppData\Local\Google\Chrome\User Data\") {
        ...
        Get-ChildItem "C:\Users\$Username\AppData\Local\Google\Chrome\User Data\" | Where-Object { `
            (Test-Path "$($_.FullName)\History") -and `
            (Test-Path "$($_.FullName)\Login Data")
        } | ForEach-Object {
            $srcpath = $_.FullName
            $destpath = $_.FullName -replace "^C:\\Users\\$Username\\AppData\\Local\\Google\\Chrome\\User Data",$HistoryFolder
...
# functions that dont need admin priv
...
# function Zip-Results and results are zipped in a folder
Zip-Results